...
def plot_compare(y_true, y_pred, trace=None, min_trace=0, max_trace=1):
    ...
    # uniform resampling
    data = pd.DataFrame({'y_true': y_true, 'y_pred': y_pred, 'trace': trace}).asfreq('30s')
    ...
    ax_plot(axs[0], y_true, ylabel='true')
    ax_plot(axs[1], y_pred, ylabel='pred')
